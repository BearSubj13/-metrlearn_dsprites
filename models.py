...
class AutoEncoderWithProjector(nn.Module):
    def __init__(self, in_channels, dec_channels, latent_size):
        #nn.Module.__init__(self)
        super().__init__()
        ...
        self.e_fc_0 = nn.Linear(dec_channels * 16 * 2 * 2, dec_channels * 16 * 2 * 2, bias=False)
        #self.e_bn_6 = nn.BatchNorm1d(dec_channels * 16 * 2 * 2)

        self.e_fc_1 = nn.Linear(dec_channels * 16 * 2 * 2, latent_size, bias=False)
